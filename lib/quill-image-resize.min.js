...
(function () {
  ...
  class ImageResize {
    constructor(quill, options) {
      ...
      // 빈 블록 자동 제거는 리사이즈 종료 시에만 실행 (text-change에서 제거하면 엔터가 안 먹힘)
    }
    // DOM 기반 빈 문단 제거
    ...
    onDown = (e) => {
      if (this.img && e.target === this.img) {
        // 드래그 시작 조건: 마우스 좌클릭 + shift/ctrl 등 modifier 없이
        if (
          e.button === 0 &&
          !e.ctrlKey &&
          !e.metaKey &&
        ...
        // 단순 클릭(드래그 아님)에는 preventDefault/stopPropagation 미적용 → 엔터 정상 동작
      }
    };
    onUp = (e) => {
      if (this.dragging) {
