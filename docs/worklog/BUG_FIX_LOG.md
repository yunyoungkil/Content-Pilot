...
**증상**:
- 이미지가 아주 약간 눌린 감이 있음
- `display: block`만으로는 완전히 해결되지 않음

**원인**:
- 인라인 요소의 기본 여백(baseline 공간)이 남아있을 수 있음
- `vertical-align` 속성이 기본값(`baseline`)으로 설정되어 있음
- `line-height`가 기본값으로 설정되어 하단 여백이 생길 수 있음

**해결 방법**:
- 이미지 태그에 추가 스타일 적용:
  - `margin: 0; padding: 0;`: 기본 여백 제거
  - `vertical-align: bottom;`: baseline 공간 제거
  - `line-height: 0;`: 줄 높이 제거로 하단 여백 완전 제거

**수정된 파일**:
- `js/services/aiService.js`: 이미지 태그 스타일 강화

**기술적 세부사항**:
- `display: block`만으로는 인라인 요소의 baseline 공간이 완전히 제거되지 않을 수 있음
- `vertical-align: bottom`과 `line-height: 0`을 함께 사용하여 모든 여백 제거
- `margin: 0; padding: 0;`으로 브라우저 기본 스타일도 제거

**테스트 방법**:
1. AI 초안 생성 시 썸네일 이미지가 눌려 보이지 않는지 확인
2. 이미지 하단에 여백이 없는지 확인
3. 다양한 브라우저에서 이미지 렌더링이 정상적인지 확인

---

### HTML 복사 시 이전 초안이 복사되는 문제 해결
...
**증상**:
- 초안을 생성하고 HTML을 복사하면, 생성 전에 생성했던 초안이 복사됨
- 최신 초안이 아닌 이전 초안이 복사되는 문제

**원인**:
- 초안 생성 완료 후 `window.__cp_workspace_idea_data`가 업데이트되지 않음
- HTML 복사 버튼이 `window.__cp_workspace_idea_data`를 참조하지만, 초안 생성 후 이 값이 갱신되지 않아 이전 데이터를 참조함
- 에디터 내용을 가져올 때 에디터가 비어있으면 저장된 draft를 사용하는데, 이 draft가 이전 초안일 수 있음

**해결 방법**:
1. **초안 생성 완료 시 `window.__cp_workspace_idea_data` 업데이트**:
   - 초안 생성 성공 후 `window.__cp_workspace_idea_data`의 `workspace.draft`, `draftContent`, `draft` 필드 업데이트
   - 발행 정보(`publishInfo`)도 함께 업데이트 (permalink, tags, seoTitle, thumbnailInfo, jsonLdSchema)
2. **HTML 복사 시 에디터 내용 우선 사용**:
   - 에디터 내용을 먼저 가져오고, 비어있을 때만 저장된 draft 사용
   - 에디터 내용이 `<p><br></p>` 또는 `<p></p>` 같은 빈 내용일 때도 저장된 draft 사용

**수정된 파일**:
- `js/ui/workspaceMode.js`: 
  - 초안 생성 완료 후 `window.__cp_workspace_idea_data` 업데이트 로직 추가
  - HTML 복사 시 에디터 내용 우선 사용 로직 개선

**기술적 세부사항**:
- `window.__cp_workspace_idea_data`는 전역 변수로 워크스페이스의 현재 아이디어 데이터를 저장
- 초안 생성 후 이 값을 업데이트하지 않으면 HTML 복사 시 이전 데이터를 참조하게 됨
- 에디터 내용을 직접 가져오는 것이 가장 정확하므로, 에디터 내용을 우선적으로 사용

**테스트 방법**:
1. 초안을 생성한 후 HTML 복사 버튼 클릭
2. 복사된 HTML이 최신 초안 내용인지 확인
3. 여러 번 초안을 생성하고 복사하여 항상 최신 초안이 복사되는지 확인

---

### 이미지 눌림 현상 추가 수정 (div 래퍼 추가)
...
**증상**:
- 이미지가 여전히 약간 눌려 보이는 현상
- 이미지 태그에 직접 스타일을 적용해도 완전히 해결되지 않음

**원인**:
- 이미지 태그 주변의 텍스트 노드나 부모 요소의 line-height가 영향을 줄 수 있음
- 이미지 태그만으로는 부모 요소의 스타일을 완전히 제어할 수 없음

**해결 방법**:
- 이미지 태그를 `div`로 감싸서 완전한 격리:
  - `div`에 `display: block; margin: 0; padding: 0; line-height: 0; font-size: 0;` 적용
  - 이미지 태그에도 기존 스타일 유지 + `border: none; outline: none; box-sizing: border-box;` 추가
  - `div` 래퍼로 인해 부모 요소의 스타일 영향을 완전히 차단

**수정된 파일**:
- `js/services/aiService.js`: 이미지 태그를 div로 감싸는 로직 추가 (삽입 시에만)

**기술적 세부사항**:
- 이미지 태그만으로는 부모 요소의 line-height, font-size 등의 영향을 받을 수 있음
- `div` 래퍼를 추가하여 이미지를 완전히 격리하고, 래퍼 자체도 모든 여백과 줄 높이를 제거
- `font-size: 0`을 추가하여 텍스트 노드의 baseline 공간도 완전히 제거

**테스트 방법**:
1. AI 초안 생성 시 썸네일 이미지가 눌려 보이지 않는지 확인
2. 이미지 하단에 여백이나 baseline 공간이 없는지 확인
3. 다양한 브라우저에서 이미지 렌더링이 정상적인지 확인

---

### 썸네일 텍스트 위치가 항상 하단에만 표시되는 문제 해결
...
**증상**:
- 썸네일 텍스트가 항상 하단에만 표시됨
- AI가 `textPosition`을 항상 "bottom"으로만 설정함
- 모든 썸네일이 같은 위치에 텍스트가 배치되어 다양성이 부족함

**원인**:
- AI 프롬프트의 예시에서 모든 컨셉이 `"textPosition": "bottom"`으로 고정되어 있음
- AI가 예시를 따라 모든 썸네일의 텍스트 위치를 "bottom"으로만 설정함
- 프롬프트에 위치 선택의 다양성을 강조하는 지시가 부족함

**해결 방법**:
1. **예시 수정**:
   - 호기심 자극형: `"textPosition": "top"`으로 변경 (상단 배치로 호기심 유발)
   - 정보 요약형: `"textPosition": "center"`로 변경 (중앙 배치로 정보 전달)
   - 감성/공감형: `"textPosition": "bottom"` 유지 (하단 배치로 감성 전달)
2. **프롬프트 개선**:
   - 위치 선택 가이드라인을 더 명확하게 제시
   - "다양성 중요"를 강조하여 모든 썸네일이 같은 위치에 오지 않도록 지시
   - 예시에서 "bottom"만 사용하지 말고 각 컨셉에 맞게 다른 위치를 선택하도록 명시

**수정된 파일**:
- `js/services/aiService.js`: 
  - 썸네일 정보 예시의 `textPosition` 값 수정 (top, center, bottom 다양화)
  - 프롬프트에 위치 선택 다양성 강조 문구 추가

**기술적 세부사항**:
- AI는 예시를 따라 학습하므로, 예시에서 다양한 위치를 보여주는 것이 중요함
- 각 컨셉 타입에 맞는 위치를 예시로 제시하여 AI가 적절한 위치를 선택하도록 유도
- 프롬프트에 "다양성 중요"를 명시하여 AI가 항상 같은 위치를 선택하지 않도록 함

**테스트 방법**:
1. 여러 번 초안을 생성하여 썸네일 텍스트 위치가 다양하게 나오는지 확인
2. "top", "center", "bottom" 위치가 모두 사용되는지 확인
3. 각 컨셉 타입에 맞는 위치가 선택되는지 확인
