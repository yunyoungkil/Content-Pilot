...
// Result 타입 클래스
export class Result {
  static success(data) { return { success: true, data }; }
  static failure(error) { return { success: false, error }; }
}
```
...
export class DraftPromptBuilder {
  ...
  build() {
    return this.sections
      .map(section => section.template(section.data))
      .join('\n\n');
  }
}
```
...
async function callDraftAPI(prompt, context) {
  ...
  return await withRetry(
    () => callGeminiAPI(optimizedPrompt),
    AI_SERVICE_CONFIG.API_RETRY
  );
}
```
