...
async function sanitizeAndFormatHtml(text) {
  ...
  const isMarkdownCandidate =
    /(^|\n)\s{0,3}(#{1,6}\s)|\*\s|\-\s|\d+\.\s|`{1,3}|\[[^\]]+\]\([^)]+\)/m.test(
      cleanedText
    );
  ...
  body.querySelectorAll("a").forEach((a) => {
    ...
    const parentSpan = a.closest(
      'span[style*="#2e7d32"], span[style*="rgb(46, 125, 50)"]'
    );

    if (parentSpan) {
      // 제휴 링크인 경우: 부모 span의 스타일을 링크에 직접 적용
      const spanStyle = parentSpan.getAttribute("style") || "";
      const colorMatch = spanStyle.match(
        /color:\s*(#[0-9a-fA-F]{6}|rgb\([^)]+\))/
      );
      if (colorMatch) {
    ...
    body.querySelectorAll("p").forEach((p) => {
      ...
      for (const keyword of importantKeywords) {
        ...
        if (sentence.length > 10 && sentence.length < 200) {
          ...
          const escapedSentence = sentence.replace(
            /[.*+?^${}()|[\]\\]/g,
            "\\$&"
          );
...
function parseContentAndMetrics(doc, urlObj) {
  ...
  if (contentElement) {
    ...
    const unnecessarySelectors =
      'ins, script, style, .adsbygoogle, [id*="ad-"], .ad-section';
    contentClone
      .querySelectorAll(unnecessarySelectors)
    ...
    images.forEach((img) => {
      ...
      const isDthumb =
        imageUrl && imageUrl.includes("dthumb-phinf.pstatic.net");
    ...
    hasVideo =
      contentClone.querySelector(
        'iframe[src*="youtube.com"], iframe[src*="vimeo.com"], video'
      ) !== null;
  ...
  if (commentCountElement) {
    commentCount = parseInt(
      commentCountElement.innerText.match(/\d+/)?.[0] || "0",
      10
    );
  }
  ...
  if (likeCountElement) {
    likeCount = parseInt(
      likeCountElement.innerText.match(/\d+/)?.[0] || "0",
      10
    );
  }
...
async function resizeImage(imageDataUrl, maxWidth, maxHeight, quality = 0.9) {
  return new Promise((resolve, reject) => {
    ...
    if (!imageDataUrl.startsWith("data:image/")) {
      reject(
        new Error(
          "DataURL 형식이 올바르지 않습니다. data:image/로 시작해야 합니다."
        )
      );
      return;
    }
    ...
    img.onerror = (error) => {
      ...
      reject(
        new Error(
          `이미지 로드 실패: DataURL이 유효하지 않거나 손상되었을 수 있습니다.`
        )
      );
    };
...
async function renderTemplateInOffscreen(
  ...
  try {
    ...
    console.log(
      `⚡ [Offscreen] 템플릿 렌더링 완료: ${canvasWidth}x${canvasHeight} (${elapsed}ms)`
    );
    return dataUrl;
  } catch (error) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === "parse_html_in_offscreen") {
    ...
    try {
      ...
      const uniqueTags = [
        ...new Set(
          metaTags.map((t) => t.trim()).filter((t) => t && t.length > 0)
        ),
      ];
