...
let isScrapListenerActive = false;
// 스크랩 실시간 리스너 (최초 1회만 활성화)
function activateScrapRealtimeListener() {
  if (isScrapListenerActive) return;
  isScrapListenerActive = true;
  firebase
    .database()
    .ref("scraps")
    .on("value", (snapshot) => {
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.action === "get_kanban_data") {
    ...
    return true; // 비동기 응답을 위해 true 반환
  } else if (msg.action === "create_new_idea_card") {
    const { title } = msg.data || {};
    if (!title || typeof title !== "string" || !title.trim()) {
      sendResponse({ success: false, error: "제목이 필요합니다." });
      return true;
    }
    const newCard = {
      title: title.trim(),
      createdAt: Date.now(),
      // tags 필드 제외, 필요시 추가 필드 확장 가능
    };
    ...
    newCardRef
      .set(newCard)
      .then(() => {
        sendResponse({ success: true, key: newCardKey, data: newCard });
      })
      .catch((e) => {
    ...
    const draftRef = firebase
      .database()
      .ref(`kanban/${status}/${ideaId}/draftContent`);
    draftRef
      .set(draft)
      .then(() => {
        sendResponse({ success: true });
