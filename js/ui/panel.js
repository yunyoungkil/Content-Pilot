    ...
    if (!window.__cp_tui_panel_listener_attached) {
        const tuiEditorPanelListener = (event) => {
            if (event.data?.action === "cp_open_tui_editor") {
                ...
                // workspace 컨테이너 찾기
                const workspaceContainer = shadowRoot.querySelector(".workspace-container") || 
                        ...
                        setTimeout(() => {
                            if (tuiEditorIframe && tuiEditorIframe.contentWindow) {
                                ...
                                if (event.data.allDocumentImages && Array.isArray(event.data.allDocumentImages) && event.data.allDocumentImages.length > 0) {
                                    ...
                                    console.log("[Panel] 문서 이미지 목록 전달 완료:", event.data.allDocumentImages.length, "개");
                                }
                        ...
                        setTimeout(() => {
                            if (tuiEditorIframe && tuiEditorIframe.contentWindow) {
                                ...
                                if (event.data.allDocumentImages && Array.isArray(event.data.allDocumentImages) && event.data.allDocumentImages.length > 0) {
                                    ...
                                    console.log("[Panel] 문서 이미지 목록 전달 완료 (즉시):", event.data.allDocumentImages.length, "개");
                                }
