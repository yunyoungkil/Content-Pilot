...
export function createAndShowPanel() {
  ...
  if (host) {
        ...
        if (event.data?.action === "cp_open_tui_editor") {
          ...
          if (!tuiEditorIframe) {
            ...
            tuiEditorIframe.onload = () => {
              console.log(
                "[Panel] TUI 에디터 iframe 로드 완료, 이미지 전달 중..."
              );
              setTimeout(() => {
            ...
            console.log(
              "[Panel] 기존 TUI 에디터 iframe 재사용, 이미지 전달 중..."
            );
            if (tuiEditorIframe.contentWindow) {
              // 1. 이미지 열기
              tuiEditorIframe.contentWindow.postMessage(
    ...
    chrome.storage.local.get(
      ["activeChannelId", "migration_completed"],
      (res) => {
        ...
        if (res.activeChannelId) {
          ...
          chrome.runtime.sendMessage(
            ...
            (response) => {
              console.log(
                "[Panel] activeChannelId 검증, get_channels_and_key 응답:",
                response
              );
              ...
              const generateChannelId = (channel) => {
                ...
                if (channel.apiUrl)
                  return btoa(channel.apiUrl).replace(/=/g, "");
              ...
              if (isValidChannel && myBlogs.length > 0) {
                ...
                console.log(
                  "[Panel] 유효한 activeChannelId 확인, 대시보드 표시"
                );
    ...
    chrome.storage.onChanged.addListener((changes, namespace) => {
      ...
      if (namespace === "local" && changes.activeChannelId) {
        ...
        const activeTabBtn = host.shadowRoot.querySelector(
          ".cp-mode-tab.active"
        );
        if (!activeTabBtn) {
          console.warn(
            "[Panel] 활성 탭을 찾을 수 없습니다. 초기 로딩 중일 수 있습니다."
          );
          return;
        }
        ...
        const renderPromise = (() => {
          if (tabName === "kanban") {
            ...
            const currentIdeaId = mainArea.querySelector(
...
function addEventListenersToPanel(shadowRoot) {
  ...
  panelContent.addEventListener("click", (e) => {
    ...
    if (tab) {
      ...
      if (previousMode) {
        Logger.debug(
          `[Panel] 모드 전환: ${previousMode} → ${activeKey}, 이전 모드 정리 중...`
        );
...
function showOnboardingMessage(container) {
  ...
  // 채널 설정 컨테이너의 맨 위에 삽입
  const channelContainer = container.querySelector(
    ".channel-settings-container"
  );
  if (channelContainer) {
    channelContainer.insertBefore(
      onboardingBanner,
      channelContainer.firstChild
    );
  } else {
