...
export function renderDraftMode(container) {
  ...
  footer.style.cssText =
    'padding: 12px; border-top: 1px solid #eee; background: #fff; display:flex; gap:8px; align-items:center;';
  footer.innerHTML = `
...
function loadStorageImages(container) {
  ...
  chrome.runtime.sendMessage({ action: 'get_unified_gallery', filter: '#Storage' }, (res) => {
    ...
    const getSelected = () =>
      Array.from(container.querySelectorAll('.storage-select-checkbox'))
