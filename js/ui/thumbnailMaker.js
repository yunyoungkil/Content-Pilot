...
export function openThumbnailMaker(draftData, onInsert, onSave, onEditTui) {
  ...
  document.getElementById("tm-gen-bg").onclick = async () => {
    ...
    try {
      ...
      if (style === "abstract") {
        promptSuffix = "abstract geometric shapes, modern, minimalistic, dark theme background, text-friendly";
      } else if (style === "gradient") {
        promptSuffix = "smooth gradient texture, grain noise, high quality wallpaper, 4k, professional";
      } else if (style === "office") {
        promptSuffix = "blurred modern office background, bokeh, professional workspace, soft lighting";
      } else if (style === "nature") {
        promptSuffix = "scenic nature background, soft lighting, cinematic, text-friendly overlay";
      } else if (style === "dark") {
  ...
  document.getElementById("tm-insert").onclick = async () => {
    ...
    try {
      ...
      // 1. 제목 가져오기
      const rawTitle = document.getElementById("tm-title").value || thumbInfo.thumbnailText || "thumbnail";
