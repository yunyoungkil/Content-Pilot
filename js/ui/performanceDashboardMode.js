  ...
  chrome.storage.local.get(['googleUserEmail'], (authResult) => {
    if (!authResult.googleUserEmail) {
      if (retryCount < MAX_RETRY_COUNT) {
        ...
        if (contentEl) contentEl.innerHTML = '<div class="perf-loading">로그인이 필요합니다.</div>';
      }
      return;
    }
...
function renderPerformanceList(container, sortBy = 'earnings-desc') {
  ...
  if (goalAchieved && !window.goalAchievedToday) {
    ...
    setTimeout(() => {
      ...
      if (typeof triggerConfettiAnimation === 'function') triggerConfettiAnimation();
  ...
  if (scrollPosition > 0) requestAnimationFrame(() => (contentEl.scrollTop = scrollPosition));
}
...
function createPerformanceCard(item, index) {
  ...
  const impressions = perf.pageImpressions ? perf.pageImpressions.toLocaleString() : '0';
  const position = perf.avgPosition ? perf.avgPosition.toFixed(1) : '-';

  // [신규] 재방문율
  const returningRatio = perf.returningUsersRatio ? (perf.returningUsersRatio * 100).toFixed(0) : 0;
    ...
    <div class="perf-card" data-card-id="${item.id}" style="${
      ...
      <div class="perf-card-content">
        ...
        <div class="perf-card-footer">
          <span class="perf-updated-time">업데이트: ${new Date(item.lastUpdatedAt).toLocaleString(
            'ko-KR'
          )}</span>
        </div>
      </div>
...
function hasDetails(perf) {
  return (
    (perf.deviceBreakdown &&
      (perf.deviceBreakdown.mobile?.users > 0 || perf.deviceBreakdown.desktop?.users > 0)) ||
...
function addPerformanceDashboardEventListeners(container) {
  ...
  if (sortSelect) {
    sortSelect.addEventListener('change', (e) => renderPerformanceList(container, e.target.value));
  }
