  ...
  container.innerHTML = `
    <div class="kanban-board-container">
      <div class="kanban-controls-header">
        ...
        <button id="create-new-draft-btn" class="kanban-new-draft-btn">+ 새 초안</button>
      </div>
...
function addKanbanEventListeners(container) {
  ...
  // 새 초안 버튼 클릭 시 제목 입력 프롬프트 호출
  const newDraftBtn = container.querySelector("#create-new-draft-btn");
  if (newDraftBtn) {
    newDraftBtn.addEventListener("click", () => {
      const draftTitle = window.prompt("새 아이디어의 제목을 입력하세요:");
      ...
      chrome.runtime.sendMessage(
        {
          action: "create_new_idea_card",
          data: {
            title: draftTitle.trim(),
          },
        },
        (response) => {
          // 생성된 카드의 key와 데이터 응답 처리
          if (response && response.success && response.data) {
            // 즉시 워크스페이스로 전환 (AI 추천 데이터가 없으면 빈 상태로 렌더)
            import("./workspaceMode.js").then((mod) => {
              mod.renderWorkspace(document.querySelector("#main-area"), {
                ...response.data,
                id: response.key, // 카드의 고유 ID
              });
            });
          }
        }
