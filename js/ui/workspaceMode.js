...
function updateImageGalleryFromAllScraps(
  resourceLibrary,
  allScraps,
  sendCommand,
  ideaData = null
  ...
  const imageGalleryGrid = imageGalleryArea.querySelector(
    ".image-gallery-grid"
  );
  const searchInput = imageGalleryArea.querySelector("#image-search-input");
  const filterByDraftBtn = imageGalleryArea.querySelector(
    "#filter-by-draft-btn"
  );
  ...
  chrome.runtime.sendMessage(
    { action: "get_canvas_images" },
    (canvasResponse) => {
      if (
        canvasResponse &&
        canvasResponse.success &&
        Array.isArray(canvasResponse.images)
      ) {
      ...
      function calculateRelevance(imageData, draftText) {
        ...
        try {
          if (imageData.url) {
            ...
            if (
              urlDomain &&
              draftText.toLowerCase().includes(urlDomain.split(".")[0])
            )
              score += 0.5;
          }
        } catch (e) {}
      ...
      function renderImages(images) {
        ...
        imageGalleryGrid.innerHTML = images
          ...
          <div class="gallery-thumb-wrap" draggable="true" data-image-url="${escapedUrl}" 
            ...
            data-timestamp="${imgData.timestamp}" data-scrap-id="${
              imgData.scrapId || ""
            }"
        ...
        imageGalleryGrid
          .querySelectorAll(".gallery-thumb-wrap")
          .forEach((wrap) => {
      ...
      function showImagePreview(imgData) {
        ...
        const date = imgData.timestamp
          ? new Date(parseInt(imgData.timestamp))
          : new Date();
...
function renderThumbnailButton(workspaceEl, ideaData) {
  const buttonContainer = workspaceEl.querySelector(
    "#workspace-title-header"
  )?.nextElementSibling;
  // 이미 버튼이 있으면 중단
  if (
    !buttonContainer ||
    buttonContainer.querySelector("#btn-create-thumbnail")
  )
    return;
  ...
  thumbBtn.onclick = () => {
    ...
    const onSave = (newThumbnailInfo) => {
      ...
      if (Array.isArray(ideaData.publishInfo.thumbnailInfo)) {
        ...
        if (
          selectedIndex >= 0 &&
          selectedIndex < ideaData.publishInfo.thumbnailInfo.length
        ) {
        ...
        if (newThumbnailInfo.selectedThumbnailIndex !== undefined) {
          ideaData.publishInfo.selectedThumbnailIndex =
            newThumbnailInfo.selectedThumbnailIndex;
        }
      }
...
function createScrapCard(scrap, isLinked) {
    ...
    return `<div class="scrap-card-item linked-scrap-item" data-scrap-id="${
      ...
      "&quot;"
        ...
        <div class="linked-scrap-tag">
          ...
          <button class="unlink-scrap-btn" data-scrap-id="${
            scrap.id
          }" title="연결 해제">×</button>
        </div>
      </div>`;
  ...
  const previewImage =
    scrap.image ||
    (Array.isArray(scrap.allImages) && scrap.allImages.length > 0
      ? scrap.allImages[0]
      : "");
  ...
  }" data-scrap-id="${scrap.id}" data-current-channel-id="${
    scrap.channelId || ""
  }" 
...
function showScrapDetailModal(scrapData, container = null) {
  ...
  if (!modal) {
    Logger.error("[Workspace] 스크랩 상세 모달을 찾을 수 없습니다.", {
      ...
      containerQuerySelector: container?.querySelector
        ? "available"
        : "not available",
    });
    return;
  ...
  // 제목
  const title = scrapData.text
    ? scrapData.text.substring(0, 100).replace(/\n/g, " ")
    : "제목 없음";
  ...
  const escHandler = (e) => {
    if (e.key === "Escape" && modal.style.display !== "none") {
      modal.style.display = "none";
      const targetDoc =
        container?.getRootNode()?.host?.ownerDocument || document;
...
function showPublishInfo(workspaceEl, permalink, tags, seoTitle, ideaData) {
  ...
  chrome.runtime.sendMessage(
    ...
    `;
      ...
      const publishInfoArea = workspaceEl.querySelector(
      ...
      if (publishInfoArea) {
        ...
        const publishInfoAreaContainer =
          workspaceEl.querySelector("#publish-info-area");
      ...
      // 이벤트 리스너
      const connectBtn = publishInfoPanel.querySelector(
        "#connect-permalink-btn"
      );
      if (connectBtn && fullUrl) {
        connectBtn.addEventListener("click", () => {
          ...
          const currentStatus =
            ideaData.status ||
            window.__cp_workspace_idea_data?.status ||
            "in-progress";
          chrome.runtime.sendMessage(
            {
      ...
      if (copyHtmlBtn) {
        copyHtmlBtn.addEventListener("click", async () => {
          try {
            ...
            // 에디터 내용 가져오기
            const editorIframe = workspaceEl.querySelector(
              "#quill-editor-iframe"
            );
            let editorHtml = "";
...
export function renderWorkspace(container, ideaData) {
  ...
  if (ideaData.linkedScraps) {
    if (
      ...
      typeof ideaData.linkedScraps === "object"
    ) {
  ...
  if (!ideaData.tags && ideaData.workspace.keywords)
    ideaData.tags = ideaData.workspace.keywords;
  ...
  if (
    ...
    Logger.debug(
      `[Workspace] 브리핑 요청 - cardId: ${ideaData.id}, title: ${ideaData.title}`
    );
    chrome.runtime
      .sendMessage({
  ...
  const mainKeywordsHtml =
    ideaData.mainKeywords?.length > 0
      ? ideaData.mainKeywords
          .map(
            (k) =>
              `<span class="tag main-keyword-tag interactive-tag">🔑 ${k}</span>`
          )
          .join("")
      : ideaData.tags?.length > 0
      ? ideaData.tags
  ...
  const longTailHtml =
    ideaData.longTailKeywords?.length > 0
      ? ideaData.longTailKeywords
          .map(
            (k) =>
              `<span class="tag long-tail-keyword interactive-tag">${k}</span>`
          )
          .join("")
  ...
  try {
    ...
    if (shadowWindow && !shadowWindow.__cp_tui_shadow_listener_attached) {
      Logger.debug(
        "🔧 [Workspace] Shadow DOM 내부 window에 TUI 에디터 리스너 등록 중..."
      );
      Logger.debug("🔧 [Workspace] Shadow window:", shadowWindow);
      Logger.debug(
        "🔧 [Workspace] Shadow window === window:",
        shadowWindow === window
      );
      ...
      const shadowTuiEditorListener = (event) => {
        if (event.data?.action === "cp_open_tui_editor") {
          ...
          console.log(
            "🌐 [Workspace] 📨 Shadow DOM 리스너: cp_open_tui_editor 메시지 수신!"
          );
          console.log(
            "🌐 [Workspace] ========================================"
          );
          ...
          if (!imageUrl) {
            Logger.error(
              "❌ [Workspace] TUI 에디터 열기 실패: 이미지 URL 없음"
            );
            return;
          }
          ...
          if (!workspaceContainer) {
            Logger.error(
              "❌ [Workspace] workspace 컨테이너를 찾을 수 없습니다!"
            );
            return;
          }
          ...
          if (!tuiEditorIframe) {
            ...
            const editorIframe = workspaceContainer.querySelector(
              "#quill-editor-iframe"
            );
            ...
            const tuiEditorMessageHandler = function (e) {
              if (e.data?.action === "tui-editor-result" && e.data.dataUrl) {
                console.log(
                  "[Workspace] TUI 에디터 편집 완료, 결과 처리 중..."
                );
                ...
                shadowWindow.removeEventListener(
                  "message",
                  tuiEditorMessageHandler
                );
              }
            };
            ...
            tuiEditorIframe.onload = () => {
              console.log(
                "[Workspace] TUI 에디터 iframe 로드 완료, 이미지 전달 중..."
              );
              setTimeout(() => {
            ...
            console.log(
              "[Workspace] 기존 TUI 에디터 iframe 재사용, 이미지 전달 중..."
            );
      ...
      Logger.debug(
        "✅ [Workspace] Shadow DOM 내부 window에 TUI 에디터 리스너 등록 완료"
      );
    } else if (shadowWindow && shadowWindow.__cp_tui_shadow_listener_attached) {
      Logger.info(
        "ℹ️ [Workspace] Shadow DOM 내부 window에 이미 리스너가 등록되어 있습니다."
      );
    }
  } catch (err) {
    console.warn(
      "⚠️ [Workspace] Shadow DOM 내부 window 리스너 등록 실패:",
      err.message
    );
  }
...
function addWorkspaceEventListeners(workspaceEl, ideaData, container = null) {
  ...
  if (
    linkedScrapsList &&
    ideaData.linkedScraps &&
    ideaData.linkedScraps.length > 0
  ) {
    chrome.storage.local.get("activeChannelId", (res) => {
      chrome.runtime.sendMessage(
        ...
        (r) => {
          if (r && r.success && r.scraps) {
            // 연결된 스크랩만 필터링
            const linkedScraps = r.scraps.filter((s) =>
            ...
            if (linkedScraps.length > 0) {
              ...
              linkedScrapsList
                .querySelectorAll(".linked-scrap-item")
                .forEach((item) => {
  ...
  tabBtns.forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (tab === "all-scraps") {
        ...
        chrome.storage.local.get("activeChannelId", (res) => {
          // 로딩 표시
          const imageGalleryGrid = resourceLibrary.querySelector(
            ".image-gallery-grid"
          );
          if (imageGalleryGrid) {
            imageGalleryGrid.innerHTML =
        ...
        chrome.runtime.sendMessage(
          ...
          (statusResponse) => {
            if (statusResponse && statusResponse.success) {
              const status =
                statusResponse.status || ideaData.status || "ideas";
                ...
                (kanbanResponse) => {
                  if (
                    ...
                    if (cardData) {
                      ...
                      if (cardData.publishInfo)
                        ideaData.publishInfo = cardData.publishInfo;
                      if (cardData.seoTitle)
                        ideaData.seoTitle = cardData.seoTitle;
        ...
        try {
          ...
          listContainer
            .querySelectorAll(".insert-affiliate-text")
            .forEach((btn) => {
          ...
          listContainer
            .querySelectorAll(".insert-affiliate-card")
            .forEach((btn) => {
              btn.addEventListener("click", (e) => {
                const id = btn.dataset.id;
                const link = links.find((l) => l.id === id);
                      ...
                      <div style="border:1px solid #eee;border-radius:12px;padding:16px;display:flex;gap:16px;max-width:640px;background:#fff;box-shadow:0 6px 20px rgba(0,0,0,0.06);">
                        ...
                        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
                          <div style="font-weight:900;color:#ae0000;font-size:18px;">${
                            cd.salePrice
                              ? Number(cd.salePrice).toLocaleString() + "원"
                              : ""
                          }</div>
                          ${
  ...
  const editorMessageListener = (event) => {
    ...
    if (event.data && typeof event.data === "object" && event.data.action) {
      console.log(
        "[Workspace] 에디터 메시지 수신:",
        event.data.action,
        event.data
      );
      console.log("[Workspace] 에디터 메시지 소스:", event.source);
      console.log(
        "[Workspace] editorIframe?.contentWindow:",
  ...
  if (editorIframe && editorIframe.contentWindow) {
    try {
      ...
      console.log(
        "[Workspace] editor iframe의 parent window:",
        editorParentWindow
      );
      console.log(
        "[Workspace] editorParentWindow === window:",
        editorParentWindow === window
      );
      console.log(
  ...
  if (!window.__cp_tui_listener_attached) {
    Logger.debug(
      "🔧 [Workspace] addWorkspaceEventListeners 내부 TUI 리스너 등록 중..."
    );
      ...
      if (event.data?.action === "cp_open_tui_editor") {
        ...
        console.log("📦 [Workspace] 메시지 데이터:", {
          ...
          imageUrl: event.data.imageUrl
            ? event.data.imageUrl.substring(0, 50) + "..."
            : "없음",
        ...
        if (!tuiEditorIframe) {
          ...
          const tuiEditorMessageHandler = function (e) {
            if (e.data?.action === "tui-editor-result" && e.data.dataUrl) {
              ...
              console.log(
                "📊 [Workspace] 결과 데이터 URL 길이:",
                e.data.dataUrl.length,
                "bytes"
              );
              ...
              if (
                ...
                if (editorIframe && editorIframe.contentWindow) {
                  ...
                  Logger.debug(
                    "✅ [Workspace] 에디터에 이미지 삽입 메시지 전송 완료"
                  );
                ...
                if (editorIframe && editorIframe.contentWindow) {
                  ...
                  Logger.debug(
                    "✅ [Workspace] 에디터에 이미지 교체 메시지 전송 완료"
                  );
                } else {
                  Logger.error("❌ [Workspace] 에디터 iframe을 찾을 수 없음");
                }
          ...
          tuiEditorIframe.onload = () => {
            ...
            setTimeout(() => {
              if (tuiEditorIframe && tuiEditorIframe.contentWindow) {
                ...
                Logger.debug(
                  "✅ [Workspace] 1️⃣ open-tui-editor 메시지 전송 완료"
                );
                ...
                console.log(
                  "🎨 [Workspace] TUI 에디터에 모든 데이터 전달 완료!"
                );
              } else {
                Logger.error(
                  "❌ [Workspace] TUI 에디터 iframe contentWindow 접근 실패"
                );
              }
            }, 500); // iframe 로드 대기
          };
          ...
          if (tuiEditorIframe.contentWindow) {
            console.log(
              "⚡ [Workspace] TUI 에디터 iframe이 이미 로드됨, 즉시 이미지 전달"
            );
            setTimeout(() => {
              if (tuiEditorIframe && tuiEditorIframe.contentWindow) {
                // 1. 이미지 열기
                ...
                Logger.debug(
                  "✅ [Workspace] 1️⃣ open-tui-editor 메시지 전송 완료 (즉시)"
                );
                ...
                console.log(
                  "🎨 [Workspace] TUI 에디터에 모든 데이터 전달 완료 (즉시)!"
                );
              }
            }, 100);
          }
          ...
          if (tuiEditorIframe.contentWindow) {
            ...
            Logger.debug(
              "✅ [Workspace] 1️⃣ open-tui-editor 메시지 전송 완료 (재사용)"
            );
            ...
            console.log(
              "🎨 [Workspace] TUI 에디터에 이미지 전달 완료 (재사용)!"
            );
          } else {
            Logger.error(
              "❌ [Workspace] 기존 TUI 에디터 iframe의 contentWindow 접근 실패"
            );
          }
        }
      }
    ...
    Logger.debug(
      "✅ [Workspace] TUI 에디터 메시지 리스너 등록 완료 (addWorkspaceEventListeners)"
    );
    try {
      Logger.debug(
        "🔍 [Workspace] 디버깅: window.location.href =",
        window.location.href
      );
    } catch (e) {
      Logger.debug(
        "🔍 [Workspace] 디버깅: window.location.href = cross-origin (접근 불가)"
      );
    }
    Logger.debug(
      "🔍 [Workspace] 디버깅: window === window.top =",
      window === window.top
    );
  } else {
    console.log(
      "[Workspace] TUI 에디터 메시지 리스너가 이미 등록되어 있습니다."
    );
  }
  ...
  if (editorParentWindow && editorParentWindow !== window) {
    try {
      ...
      console.log(
        "[Workspace] TUI 에디터 메시지 리스너 등록 완료 (editor parent window)"
      );
    } catch (e) {
      console.warn(
        "[Workspace] editor parent window에 메시지 리스너 등록 실패:",
        e
      );
    }
  }
  ...
  if (
    window.top &&
    window.top !== window &&
    window.top !== editorParentWindow
  ) {
    try {
      window.top.addEventListener("message", tuiEditorMessageListener);
      console.log(
        "[Workspace] TUI 에디터 메시지 리스너 등록 완료 (window.top)"
      );
    } catch (e) {
    ...
    try {
      ...
      editorIframe.addEventListener("load", () => {
        try {
          ...
          console.warn(
            "[Workspace] iframe load 후 parent window 접근 실패:",
            e
          );
        }
      });
    ...
    if (
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            ...
            if (response && response.success) {
              ...
              showToast(
                "❌ 변경 실패: " + (response?.error || "알 수 없는 오류")
              );
            }
          }
        );
            ...
            if (response && response.success && response.draft) {
              ...
              if (
                ...
                if (response.permalink)
                  publishInfoUpdates.permalink = response.permalink;
                  ...
                  chrome.runtime.sendMessage(
                    ...
                    (updateResponse) => {
                      if (updateResponse && !updateResponse.success) {
                        console.error(
                          "[Workspace] 발행 정보 Firebase 저장 실패:",
                          updateResponse
                        );
                      }
                    }
                ...
                if (!ideaData.publishInfo) ideaData.publishInfo = {};
                if (response.permalink)
                  ideaData.publishInfo.permalink = response.permalink;
                ...
                setTimeout(() => {
                  const publishInfo = {
                    ...
                    thumbnailInfo:
                      response.thumbnailInfo ||
                      ideaData.publishInfo?.thumbnailInfo,
                  };
              ...
              if (
                buttonContainer &&
                !buttonContainer.querySelector("#delete-draft-in-workspace")
              ) {
      ...
      if (confirm("초안을 삭제하시겠습니까?")) {
        ...
        if (!editorIframe) {
          ...
          console.error(
            "[Workspace] 에디터 iframe의 contentWindow에 접근할 수 없습니다."
          );
        } else {
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            ...
            if (response && response.success) {
              ...
              Logger.debug(
                `[Workspace] 초안 삭제 후 자동 저장 차단 시작 (5초)`
              );
              ...
              const thumbBtn = workspaceEl.querySelector(
                "#btn-create-thumbnail"
              );
              ...
              if (deleteBtn && deleteBtn.parentNode) {
                deleteBtn.remove();
                console.log(
                  "[Workspace] 초안 삭제 버튼이 UI에서 제거되었습니다."
                );
              }
      ...
      if (
        e.target.closest(".unlink-scrap-btn") ||
        e.target.closest(".scrap-card-delete-btn")
      ) {
        return;
      }
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            if (response && response.success && response.data) {
              ...
              console.warn(
                "[Workspace] 스크랩 상세 데이터 가져오기 실패, dataset 정보 사용"
              );
              const fallbackData = {
                text:
                  card.dataset.text ||
                ...
                allImages: card.dataset.previewImage
                  ? [card.dataset.previewImage]
                  : [],
                tags: [],
              };
              // shadow DOM 내부의 모달을 찾기 위해 container 전달
  ...
  if (allScrapsList) {
    ...
    allScrapsList.addEventListener("click", (e) => {
      ...
      showConfirmationToast("정말로 스크랩을 삭제하시겠습니까?", () => {
        chrome.runtime.sendMessage(
          { action: "delete_scrap", id: scrapId },
          (response) => {
            if (chrome.runtime.lastError) {
              console.error(
                "[Workspace] 스크랩 삭제 오류:",
              ...
              showToast(
                `❌ 삭제 실패: ${chrome.runtime.lastError.message}`,
                "error"
              );
              return;
            }
  ...
  workspaceEl.addEventListener("dragover", (e) => {
    ...
    if (
      linkedScrapsList &&
      (linkedScrapsList.contains(e.target) ||
        e.target.closest(".linked-scraps-list"))
    ) {
      ...
      chrome.runtime.sendMessage(
        ...
        (res) => {
          if (res && res.success) {
            ...
            linkedScrapsList.classList.remove("empty-state");
            linkedScrapsList.insertAdjacentHTML(
              "beforeend",
              createScrapCard(scrapData, true)
            );
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (response && response.success) {
          ...
          showToast(
            `❌ 목차 업데이트 실패: ${response?.error || "알 수 없는 오류"}`
          );
        }
      }
    );
  ...
  // 스크랩 초안 필터 기능
  const filterScrapByDraftBtn = workspaceEl.querySelector(
    "#filter-scrap-by-draft-btn"
  );
  ...
  function getFilterTextFromIdeaData() {
    ...
    if (
      ideaData.recommendedKeywords &&
      Array.isArray(ideaData.recommendedKeywords)
    ) {
  ...
  function updateScrapList() {
    if (
      ...
      window.__cp_scrap_filter.allScraps.length === 0
    )
      return;

    const searchTerm = scrapSearchInput
      ? scrapSearchInput.value.toLowerCase().trim()
      : "";
...
window.__cp_updateScrapList = function (
  filtered,
  allCont,
  linkedCont,
  ideaData
  ...
  if (filtered.length > 0) {
    ...
    const hasSearch = searchInput && searchInput.value.trim();
    const hasFilter =
      filterBtn && filterBtn.style.background === "rgb(232, 240, 254)";
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (response && response.success && response.scraps) {
            // 연결된 스크랩은 제외하고 표시
            const linkedScrapIds = ideaData.linkedScraps || [];
            const availableScraps = response.scraps.filter(
              (s) => !linkedScrapIds.includes(s.id)
            );
...
(function () {
  ...
  if (window.__cp_tui_global_listener_attached) {
    Logger.info(
      "ℹ️ [Workspace] 전역 TUI 에디터 메시지 리스너가 이미 등록되어 있습니다."
    );
    return;
  }
  ...
  const globalTuiEditorMessageListener = (event) => {
    ...
    if (event.data?.action === "cp_open_tui_editor") {
      console.log("🌐 [Workspace] ========================================");
      console.log(
        "🌐 [Workspace] 📨 전역 리스너: cp_open_tui_editor 메시지 수신!"
      );
      console.log("🌐 [Workspace] ========================================");
      ...
      if (!tuiEditorIframe) {
        ...
        const tuiEditorMessageHandler = function (e) {
          if (e.data?.action === "tui-editor-result" && e.data.dataUrl) {
            ...
            console.log(
              "🎉 [Workspace] ✨ TUI 에디터 편집 완료! (전역 리스너)"
            );
            console.log(
              "🎉 [Workspace] ========================================"
            );
        ...
        window.addEventListener("message", tuiEditorMessageHandler);
        console.log(
          "👂 [Workspace] TUI 에디터 결과 메시지 리스너 등록 완료 (전역)"
        );
        ...
        tuiEditorIframe.onload = () => {
          ...
          setTimeout(() => {
            if (tuiEditorIframe && tuiEditorIframe.contentWindow) {
              ...
              console.log(
                "✅ [Workspace] 1️⃣ open-tui-editor 메시지 전송 완료 (전역)"
              );
              ...
              console.log(
                "🎨 [Workspace] TUI 에디터에 모든 데이터 전달 완료! (전역)"
              );
            }
          }, 500);
        };
        ...
        if (tuiEditorIframe.contentWindow) {
          console.log(
            "⚡ [Workspace] TUI 에디터 iframe이 이미 로드됨, 즉시 이미지 전달 (전역)"
          );
          setTimeout(() => {
            if (tuiEditorIframe && tuiEditorIframe.contentWindow) {
              // 1. 이미지 열기
              ...
              console.log(
                "✅ [Workspace] 1️⃣ open-tui-editor 메시지 전송 완료 (즉시, 전역)"
              );
              ...
              console.log(
                "🎨 [Workspace] TUI 에디터에 모든 데이터 전달 완료 (즉시, 전역)!"
              );
            }
          }, 100);
        }
        ...
        if (tuiEditorIframe.contentWindow) {
          ...
          console.log(
            "✅ [Workspace] 1️⃣ open-tui-editor 메시지 전송 완료 (재사용, 전역)"
          );
          ...
          console.log(
            "🎨 [Workspace] TUI 에디터에 이미지 전달 완료 (재사용, 전역)!"
          );
        }
      }
    }
