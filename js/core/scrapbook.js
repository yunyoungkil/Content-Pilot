...
export function extractContextualContent(element) {
  ...
  const sentences = fullText.split(sentenceRegex)
    .map(s => s.trim())
  ...
  sentences.forEach((sentenceText, index) => {
    ...
    const matchingNode = textNodes.find(node => {
      ...
      return nodeStart !== -1 && nodeStart <= startIndex && (nodeStart + node.text.length) >= endIndex;
    });
    ...
    const importantKeywords = ['핵심', '중요', '결론', '요약', '정리', '비교', '분석', '결과', '방법', '팁', '꿀팁', '비밀', '주의', '필수'];
    ...
    // 5. 질문 형태 (독자 관심 유도)
    if (sentenceText.includes('?') || sentenceText.includes('어떻게') || sentenceText.includes('왜') || sentenceText.includes('무엇')) score += 1;
  ...
  const topCount = Math.max(1, Math.min(
    highlights.length, 
    Math.max(1, Math.ceil(highlights.length * 0.3)), 
    10
  ));
  ...
  if (topHighlights.length > 0) {
    ...
    if (highlights.length > 0) {
      ...
      console.log('[extractContextualContent] 최장 문장 선택:', longest.text.substring(0, 50) + '...');
    }
  }
