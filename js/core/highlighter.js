...
document.addEventListener('mouseover', function (e) {
  ...
  chrome.storage.local.get(["isScrapingActive", "highlightToggleState"], function (result) {
    ...
    if (result.isScrapingActive && result.highlightToggleState) {
      ...
      if (target && !target.closest('#content-pilot-panel') && target !== document.body && lastHighlightedElement !== target) {
        clearHighlight();
...
document.addEventListener('click', function (e) {
  ...
  chrome.storage.local.get(["isScrapingActive", "highlightToggleState"], function (result) {
    // !e.altKey 대신 !result.highlightToggleState를 확인
    if (!result.isScrapingActive || !result.highlightToggleState || !lastHighlightedElement) return;
    ...
    const targetElement = lastHighlightedElement;
      ...
      images = images.concat(Array.from(imgEls).map(img => img.src).filter(Boolean));
      ...
      chrome.runtime.sendMessage({ action: "scrap_element", data: scrapData }, 
        (response) => {
          const messageAction = response && response.success ? 'cp_show_preview' : 'cp_show_toast';
          const messageData = response && response.success ? scrapData : { message: '❌ 스크랩 실패' };
          window.top.postMessage({ action: messageAction, data: messageData }, '*');
        }
      );
