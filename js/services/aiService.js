    ...
    const prompt = `
            ...
            **[균형 유지]**: 너무 과장되거나 선정적이지 않되, 독자가 지루하지 않게 적절한 재미 요소 배치. 신뢰성을 해치지 않는 선에서 흥미 유발!
            ...
            ${
              myPastPostsText
                ? `
            🔗 **[내부 링크 삽입 규칙 - 필수 적용, SEO 매우 중요, 절대 생략 금지]**
            
            🚨 **[최우선 규칙]**: 내 과거 포스팅 목록이 제공되었다면, 현재 글과 관련성이 높은 과거 글에 대해 **반드시 최소 2개 이상, 최대 3개의 내부 링크를 삽입**해주세요!
            
            ⚠️ **[경고]**: 내부 링크를 삽입하지 않으면 SEO 점수가 크게 낮아지고, 블로그 전체 가치가 하락합니다!
            
            위에 제공된 **"내 과거 포스팅 목록 (스마트 내부 링크 추천)"**을 반드시 확인하고, 관련성 높은 글 2~3개를 본문에 링크하세요.
            
            📋 **[내부 링크 삽입 필수 가이드]**:
            
            **1. 필수 삽입 개수**:
               - **최소 2개 이상, 최대 3개** 내부 링크를 반드시 삽입
               - 관련 글이 없다고 생각하지 말고, 비슷한 주제나 관련 키워드가 있는 글을 찾으세요
               - ❌ 절대 금지: 내부 링크를 하나도 삽입하지 않는 것
            ...
            **중요**: 내부 링크는 SEO 점수를 높이고, 독자의 체류 시간을 늘리며, 블로그 전체 가치를 향상시킵니다. 반드시 2~3개를 삽입하세요!
            `
                : ''
            }
