...
export async function generateThumbnailTexts(outlines, draft = '') {
  ...
  try {
    const prompt = `아래는 콘텐츠 초안과 세션(아웃라인) 목록입니다.\n\n[초안]\n${draft}\n\n[세션 목록]\n${outlines
      ...
      .join(
        '\n'
      )}\n\n각 세션에 어울리는 썸네일용 짧은 슬로건을 한글로 1개씩, 12자 이내로 추천해줘.\n\n**중요 규칙:**\n1. 단순히 제목을 그대로 사용하지 말고, 해당 세션의 핵심 메시지를 담은 독창적인 문구를 만들어줘\n2. 구두점만 있는 텍스트(예: "?", "!!", "...")는 절대 사용 금지\n3. 호기심을 유발하거나 감정을 자극하는 짧은 문구로 작성\n4. 각 슬로건은 반드시 한글, 영문, 또는 숫자를 최소 1개 이상 포함해야 함\n5. 꾸밈말 없이 핵심만 간결하게 작성\n\n좋은 예시: ["집 전체를 손끝으로", "미래가 온다", "이것만 있으면 끝"]\n나쁜 예시: ["?", "!!", "스마트홈 컨트롤"]\n\n배열 형식으로만 응답해줘: ["슬로건1", "슬로건2", ...]`;
