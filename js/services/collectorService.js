    ...
    if (indexSnap.exists()) {
      ...
      for (const match of matches) {
        ...
        if (cardSnap.exists()) {
          ...
          // 실제 카드가 존재하면 중복으로 판단
          Logger.debug(`[checkDuplicateUrl] 중복 발견 - cardId: ${match.cardId}, status: ${match.status}, title: ${cardData.title}`);
          return { 
            exists: true, 
            status: match.status, 
            cardId: match.cardId, 
          ...
          Logger.warn(`[checkDuplicateUrl] 고아 인덱스 발견 - cardId: ${match.cardId}, 인덱스에서 제거`);
